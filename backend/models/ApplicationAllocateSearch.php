 ...
 */
class ApplicationAllocateSearch extends Application
{
    public $date;
    public $date2;
    /**
    ...
    {
        return [
            [['application_status', 'project', 'family_member'], 'integer'],
            [['appl_no', 'room_no', 'start_date', 'chi_name', 'eng_name', 'mobile'], 'safe'],
            [['date', 'date2'], 'safe'],
        ];
    }
    ...
    public function search($params, $action = false)
    {
        ...
        $query = Application::find()->where(['>=','application_status', Application::APPL_STATUS_FILES_PASSED])
                                    ->andWhere(['approved' => Application::APPROVED]);
        ...
        $query->andFilterWhere(['=', new Expression('DATE(start_date)'), $this->start_date]);
        ...
        $query->andFilterWhere(['like', 'appl_no', $this->appl_no])
            ->andFilterWhere(['like', 'chi_name', $this->chi_name])
            ->andFilterWhere(['like', 'eng_name', $this->eng_name])
            ->andFilterWhere(['like', 'mobile', $this->mobile])
