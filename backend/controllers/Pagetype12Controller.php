...
use backend\models\PageType12Search;
...
class Pagetype12Controller extends Controller
{
    ...
    public function behaviors()
    {
        return [
            ...
            'access' => [
              ...
              'rules' => [
                  [
                    'actions' => ['index', 'create', 'update', 'delete', 'enabled', 'disabled', 'category', 'category_create', 'category_update', 'category_delete', 'open', 'close'],
                      'allow' => true,
                      'roles' => ['page.project'],
                  ],
              ],
...
*/
      ...
      $searchModel = new PageType12Search();
...
//      $model->MID = $menu_model->id;
      ...
      if ($model->load(Yii::$app->request->post())) {
          ...
          $model->poster_file = \yii\web\UploadedFile::getInstance($model, 'poster_file');
          if ($model->saveContent()) {
              Yii::$app->adminLog->insert(Yii::t('log', 'Add {record} to {model}', ['record' => $model->title.('[#'.$model->id.']'), 'model' => $menu_model->name.('[#'.$menu_model->id.']')]), $menu_model->id);
                Yii::$app->adminLog->insert(Yii::t('log', 'Add {record} to {model}', ['record' => '樂屋項目 [#'.$model->id.']', 'model' => '樂屋項目']));
...
//      $MID = $model->MID;
      ...
      if ($model->load(Yii::$app->request->post())) {
          ...
          $model->poster_file = \yii\web\UploadedFile::getInstance($model, 'poster_file');
          if ($model->saveContent()) {
              Yii::$app->adminLog->insert(Yii::t('log', 'Update {record} of {model}', ['record' => '樂屋項目 [#'.$model->id.']', 'model' => '樂屋項目']));
    ...
    public function actionDelete($id)
    {
      ...
      Yii::$app->adminLog->insert(Yii::t('log', 'Delete {record} of {model}', ['record' => '樂屋項目 [#'.$model->id.']', 'model' => '樂屋項目']));
    ...
    public function actionEnabled($id)
    {
        ...
        Yii::$app->adminLog->insert(Yii::t('log', 'Enable {record} of {model}', ['record' => '樂屋項目 [#'.$model->id.']', 'model' => '樂屋項目']));
    ...
    public function actionDisabled($id)
    {
        ...
        Yii::$app->adminLog->insert(Yii::t('log', 'Disable {record} of {model}', ['record' => '樂屋項目 [#'.$model->id.']', 'model' => '樂屋項目']));
    ...
    public function actionOpen($id)
    {
        ...
        Yii::$app->adminLog->insert(Yii::t('log', 'Open {record} of {model}', ['record' => '樂屋項目 [#'.$model->id.']', 'model' => '樂屋項目']));
    ...
    public function actionClose($id)
    {
        ...
        Yii::$app->adminLog->insert(Yii::t('log', 'Close {record} of {model}', ['record' => '樂屋項目 [#'.$model->id.']', 'model' => '樂屋項目']));
    ...
    {
    $model = new \backend\models\PageType12Reorder;
    // $model->category_id = $id;
    $model->setContent();

    if ($model->load(Yii::$app->request->post())  && $model->saveContent()) {
        Yii::$app->adminLog->insert(Yii::t('log', 'Update {record} of {model}', ['record' => Yii::t('log', 'Order'), 'model' => '樂屋項目']));
    ...
    private function findMenu($id) {
        $model = \common\models\Menu::find()->where(['id' => $id, 'page_type' => 12])->one();
    ...
    private function findPageType12($id) {
        $model = PageType12::findOne($id);
