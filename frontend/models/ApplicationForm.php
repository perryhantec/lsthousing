...
class ApplicationForm extends Application
{
    ...
    public function submit()
    {
        ...
        // }
        if ($this->validate()) {
            // $this->status = self::STATUS_WAITING_FOR_PROCESS;
            $this->user_id = Yii::$app->user->id;
            $this->application_status = self::APPL_STATUS_SUBMITED_FORM;

            if ($this->house_type != 6) {
                $this->house_type_other = NULL;
            }

            if ($this->house_type != 1) {
                $this->private_type = NULL;
            }

            if ($this->house_type != 4) {
                $this->together_type = NULL;
                $this->together_type_other = NULL;
            }

            if ($this->together_type != 2) {
                $this->together_type_other = NULL;
            }

            if ($this->prh != 1) {
                $this->prh_no = NULL;
                $this->prh_location = NULL;
                $this->apply_prh_year = NULL;
                $this->apply_prh_month = NULL;
                $this->apply_prh_day = NULL;
            }
            ...
            if ($this->save(false)) {
                ...
                // $this->save(true, ['appl_no']);
                $this->updateAttributes(['appl_no']);

                $this->afterFind();
                $this->updateUser();

                if ($this->create_form) {
                    $this->setApplicationMark();
                }
            }
