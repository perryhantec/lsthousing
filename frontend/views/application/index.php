...
<?php } ?>
            <div id="application" class="application content">
                <div class="pull-right"><span style="color: #b20000;">*</span>號欄位, 必須填寫</div>
                <h3><?= $menu_model->name ?></h3>

                <?= Alert::widget() ?>
                
<?php if ($user->user_appl_status <= User::USER_APPL_STATUS_UNALLOCATE_UNIT && !$user->application) { ?>
<?php $form = ActiveForm::begin([
    ...
    $fields = $model->allApplicationFormField;

    foreach ($fields as $field) {
        // if ($field == 'house_type') {
            // echo $model->$field.'ppp<br />';
            // echo $user->$field.'qqq<br />';
            // $model->$field = $model->$field ? $model->$field : isset($user->$field) ? $user->$field : '';
            $model->$field = isset($model->$field) ? $model->$field : (isset($user->$field) ? $user->$field : '');
            // echo $model->$field.'rrr<br />';
        // }
    }
            
?>
                <h1 class="text-center">樂善堂社會房屋計劃 －「樂屋」申請表格</h1>
                <h4>注意事項：</h4>
                <ol>
                    <li>申請人必須先了解本計劃之申請資格、流程及評審準則，如有疑問請與本堂同事查詢。</li>
                    <li>必須填報申請表所需的一切資料及根據「證明文件清單」提供有關證明文件和聲明書，否則申請表將會被退回，而登記將會受到延誤。</li>
                    <!-- <li>本計劃只接受親身提交、郵寄（請於信封面註明計劃名稱）及電郵形式(housing@loksintong.org)遞交申請表格 接受傳真及其他形式。</li> -->
                    <li>機構收到申請後，<span class="red"><u>不會</u></span>發出通知書或通知申請人，如需補充或查詢資料，會聯絡個別申請人。</li>
                    <li>九龍樂善堂 保留及擁有房屋編配的最終決定權。</li>
                </ol>
                <hr />
                <section>
...
<?php
    if (isset($_GET['preferred']) && (int)$_GET['preferred'] > 0) {
        $model->priority_1 = $_GET['preferred'];
    }
?>
                    <?= $form->field($model, 'priority_1')->dropdownList(Definitions::getOpenProjectName(),['prompt'=>Yii::t('app', 'Please Select')]); ?>
                    <?= $form->field($model, 'priority_2')->dropdownList(Definitions::getOpenProjectName(),['prompt'=>Yii::t('app', 'Please Select')]); ?>
                    <?= $form->field($model, 'priority_3')->dropdownList(Definitions::getOpenProjectName(),['prompt'=>Yii::t('app', 'Please Select')]); ?>
