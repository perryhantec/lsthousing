    ...
    <div class="headline">
        <div class="row">
            ...
            <div class="col-xs-4 col-xs-push-8 col-sm-push-0 col-sm-3 col-md-2">
                <?= Html::dropDownList('year', $searchModel->year, Definitions::getPageType4DisplayAtAllYear($searchModel->MID), ['class' => 'form-control', 'prompt' => Yii::t('app', '- All -'), 'onchange' => 'window.location="?year="+this.value']) ?>
            </div>
        </div>
    </div>
</div>
...
<!--<div class="row pull-right">
  Page: <?= $current_page?>/<?= $page_count?> &nbsp; Total: <?=$total_count?>
    ...
    <?php foreach($model_pt4 as $model):?>
      <!-- Post Content -->
      <div id="post-<?= $model->id ?>" class=" post-content">
        <div class="post-title"><?= Html::a($model->title, [($current_route.'/detail'), 'id' => $model->id]) ?></div>
<?php if ($current_route == 'press-release') { ?>
...
<?php } else if ($model->image_file_name != '') { ?>
            <div class="post-image"><?= Html::a(Html::img($model->image_file_name), [($current_route.'/detail'), 'id' => $model->id]) ?></div>
<?php } ?>
          </div>
...
<?php } ?>
            ...
            <div class="post-summary">
                ...
                <div class="post-more"><?= Html::a(Yii::t('web', 'Detail'), [($current_route.'/detail'), 'id' => $model->id]) ?></div>
            </div>
<?php if ($model->youtube_id != '' || $model->image_file_name != '') { ?>
          </div>
        </div>
